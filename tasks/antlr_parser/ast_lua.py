...
def print_ast(node, level=0, is_last=True, prefix=""):
    ...
    if node_type == "prog":
        ...
        for i, stmt in enumerate(node[1]):
            print_ast(stmt, level + 1, i == len(node[1]) - 1, child_prefix)

    elif node_type == "function":
        print(f"{current_prefix}function {node[1]} {node[2]}")
        print_ast(node[3], level + 1, True, child_prefix)
        ...
        print_ast(node[1], level + 1, True, child_prefix)

    elif node_type == "assign":
        print(f"{current_prefix}assign {node[1]}")
        print_ast(node[2], level + 1, True, child_prefix)

    elif node_type == "call":
        print(f"{current_prefix}call {node[1]}")
        for i, arg in enumerate(node[2]):
            print_ast(arg, level + 1, i == len(node[2]) - 1, child_prefix)

    elif node_type == "binop":
        print(f"{current_prefix}{node[1]}")
        print_ast(node[2], level + 1, False, child_prefix)
        print_ast(node[3], level + 1, True, child_prefix)
            ...
            if len(node) == 2:
                ...
                if isinstance(node[1], list):
                    for i, item in enumerate(node[1]):
                        print_ast(item, level + 1, i == len(node[1]) - 1, child_prefix)
                else:
                    print_ast(node[1], level + 1, True, child_prefix)
            else:
